 ...
 *
 */
abstract class MongoDaoController[T <: Identifiable] extends InjectedController {
  implicit val executionContext: ExecutionContext
  val dao:IdentifiableDAO[T]

  val defaultOffset = 0
  val defaultLimit = 25
  val invalidJSON = BadRequest("invalid json")
  ...
  def list(p:Int, ipp: Int, q: Option[String], s: Option[String])(implicit reader: BSONDocumentReader[T], fmt: Format[T], pgfmt: Format[Pagination], daoData:DaoData[T], tag : TypeTag[T] ) = Action.async {
    request => // get all the params that are not know params
      ...
      p match {
        case -1 => dao.findAll(q, s, paramMap).map {
          docs =>
            Ok(Json.toJson(docs))
          }
        case _ =>       dao.find(q, s, p, ipp, paramMap).map {
            docs =>
              Ok(Json.toJson(Map("pagination" -> Json.toJson(new Pagination(p,ipp,docs._2)), "items" -> Json.toJson(docs._1))))
          }
      }
  }
